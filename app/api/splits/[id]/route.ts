...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  ...
  const [participant] = session
    ? await db.select({ shareLunas: splitParticipants.shareLunas, status: splitParticipants.status }).from(splitParticipants).where(and(eq(splitParticipants.splitId, params.id), eq(splitParticipants.participantWallet, session.walletAddress))).limit(1)
    : [];
...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  const check = await verifyBasicPayment({
    ...
    expectedRecipient: split.creatorWallet,
    expectedValueLunas: participant.shareLunas,
  });
  if (check === "mismatch") {
    return Response.json({ error: "That transaction does not match your share amount or the split creator." }, { status: 400 });
  }

  const status = check === "ok" ? "paid" : "submitted";
  await db.update(splitParticipants).set({ status, paidTransactionHash: hash.slice(0, 160) }).where(eq(splitParticipants.id, participant.id));
  await db.insert(activity).values({
    id: crypto.randomUUID(),
    walletAddress: session.walletAddress,
    kind: "split-payment",
    title: `Paid split: ${split.note}`,
