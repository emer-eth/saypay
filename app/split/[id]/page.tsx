...
type SplitDetail = { id: string; creatorWallet: string; creatorHandle: string; note: string; currency: string; status: string };
...
export default function SplitPaymentPage() {
  ...
  async function payShare() {
    ...
    try {
      ...
      const participantPayload = await participantResponse.json() as { split?: SplitDetail & { participant?: { shareLunas: number } } };
      ...
      const transaction = await nimiq.sendBasicTransactionWithData({ recipient: detail.creatorWallet, value: shareLunas, data: `SayPay split:${detail.id}` });
      if (typeof transaction !== "string") throw new Error("Nimiq Pay did not return a transaction result.");
      const response = await fetch(`/api/splits/${detail.id}`, { method: "PATCH", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ transactionReference: transaction }) });
      const result = await response.json() as { error?: string };
      if (!response.ok) throw new Error(result.error ?? "Split payment was submitted but its status could not update.");
      setMessage("Your split payment was submitted through Nimiq Pay.");
    } catch (error) {
  ...
  return <main className="public-request"><section><div className="public-brand">SayPay</div>{detail ? <><p className="eyebrow">SPLIT INVITATION</p><h1>Settle your share</h1><p className="request-to">Created by <strong>@{detail.creatorHandle}</strong></p><div className="request-note">{detail.note}</div><button className="primary" onClick={payShare} disabled={paying}>{paying ? "Opening Nimiq Pay…" : "Pay your share with Nimiq Pay"}</button></> : <h1>Split invitation</h1>}<p className="public-status">{message}</p></section></main>;
}
